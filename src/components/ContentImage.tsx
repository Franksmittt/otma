...
export function ContentImageThumb({ src, alt, className = "" }: ContentImageThumbProps) {
  return (
    <div
      className={`relative aspect-[16/10] w-full min-w-0 shrink-0 overflow-hidden rounded-t-xl border-b border-zinc-200 bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-800 sm:aspect-[5/4] sm:w-44 sm:rounded-l-xl sm:rounded-tr-none sm:border-b-0 sm:border-r md:w-52 ${className}`}
    >
      <Image
        src={src}
        alt={alt}
